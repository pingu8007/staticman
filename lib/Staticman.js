...
class Staticman {
  constructor (parameters) {
    return (async () => {
      ...
      // FIXME in this stage the GitService instance can't access oauthToken, and PAT or AppKey is required.
  ...
  async _checkAuth () {
    ...
    } else if (this.options['auth-data']) {
      return this._checkAuthDelegated()
    } else {
      return this._checkAuthLegacy()
    }
  }

  async _checkAuthDelegated() {
    if (!this.siteConfig.get('auth.required')) {
      return Promise.resolve(false);
    }

    if (!this.options['auth-data']) {
      return Promise.reject(errorHandler('AUTH_DATA_MISSING'))
    }

    const authData = RSA.decrypt(this.options['auth-data'])

    if (!authData) {
      return Promise.reject(errorHandler('AUTH_DATA_INVALID'))
    }

    this.gitUser = JSON.parse(authData)
    return Promise.resolve(true)
  }

  async _checkAuthLegacy () {
    if (!this.siteConfig.get('auth.required')) {
      return Promise.resolve(false)
    }
